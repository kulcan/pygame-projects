...
from game import SnakeGameAI, Direction, BLOCK_SIZE
...
class Agent:
    ...
    def get_state(self, game):
        ...
        point_l = head + (-BLOCK_SIZE,0)
        point_r = head + (BLOCK_SIZE,0)
        point_u = head + (0,-BLOCK_SIZE)
        point_d = head + (0,BLOCK_SIZE)
    ...
    def get_action(self, state):
        # random moves: tradeoff bt exploration and explotation
...
def train():
    ...
    while True:
        ...
        if done:
            # traint long memory, plot result
            game.reset()
            agent.n_games += 1
